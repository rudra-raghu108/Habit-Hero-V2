...
export default function Motivation() {
  ...
  useEffect(() => {
    ...
    const quoteIndex = Math.abs(today.split('').reduce((a, b) => a + b.charCodeAt(0), 0)) % MOTIVATIONAL_QUOTES.length;
    const affirmationIndex = Math.abs(today.split('').reduce((a, b) => a + b.charCodeAt(0), 0)) % AFFIRMATIONS.length;
  ...
  const weeklyStats = () => {
    ...
    const weeklyCompletionRate = totalPossible > 0 ? Math.round((totalCompleted / totalPossible) * 100) : 0;
    const bestDay = Object.entries(dailyCompletions).reduce((a, b) => a[1] > b[1] ? a : b, ['', 0]);
    const averageDaily = Math.round(totalCompleted / 7);
    
    return {
      completionRate: weeklyCompletionRate,
      totalCompleted,
      bestDay: bestDay[0] ? new Date(bestDay[0]).toLocaleDateString('en-US', { weekday: 'long' }) : 'N/A',
  ...
  const totalHabits = habits.reduce((sum, habit) => sum + habit.totalCompleted, 0);

  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold">Motivation Center</h1>
        <p className="text-muted-foreground">Stay inspired and celebrate your progress</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-warning">{stats.averageDaily}</div>
              <div className="text-sm text-muted-foreground">Daily Average</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-streak">{stats.bestDayCount}</div>
          ...
          <div className="grid md:grid-cols-2 gap-4 pt-4 border-t">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{longestStreak} days</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{totalHabits} habits</div>
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-4">
          {stats.completionRate >= 80 && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <h3 className="font-semibold text-green-800">🌟 Outstanding Performance!</h3>
              <p className="text-green-700">You're crushing it this week! Your consistency is truly impressive.</p>
            </div>
          )}
          ...
          {stats.completionRate >= 50 && stats.completionRate < 80 && (
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p className="text-blue-700">You're building strong momentum. Keep up the excellent work!</p>
            </div>
          )}
          
          {stats.completionRate < 50 && (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <h3 className="font-semibold text-yellow-800">🚀 Every Step Counts!</h3>
              <p className="text-yellow-700">Remember, building habits is a journey. Focus on progress, not perfection!</p>
            </div>
          )}
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="p-3 border rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">
                Try habit stacking: attach a new habit to an existing one. For example, "After I brush my teeth, I will meditate for 2 minutes."
              </p>
            </div>
            <div className="p-3 border rounded-lg">
              <h4 className="font-medium mb-2">🎯 Challenge</h4>
              <p className="text-sm text-muted-foreground">
                Can you maintain all your habits for 3 consecutive days this week? Small wins lead to big victories!
              </p>
            </div>
          </div>
        </CardContent>
